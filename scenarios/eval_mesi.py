...
RAW_METRICS_CSV = OUTPUT_ROOT / "evaluation_metrics_mesi_raw.csv"
PLOTTABLE_METRICS_CSV = OUTPUT_ROOT / "evaluation_metrics_plot_ready_mesi.csv"
SUMMARY_METRICS_CSV = OUTPUT_ROOT / "evaluation_metrics_summary_mesi.csv"
RAW_JSON = OUTPUT_ROOT / "evaluation_metrics_raw_mesi.json"
...
def _build_protocol(
    protocol_name: str, judge: JudgeConfig, bus_latency: int
) -> ConsistencyProtocol:
    ...
    if protocol_name == "mesi":
        return MesiProtocol(bus_latency=bus_latency)
...
def _setup_simulator(
    ...
    return Simulator(
        ...
        protocol=_build_protocol(protocol_name, judge, bus_latency),
    )
...
def _run_once(
    ...
    sim = _setup_simulator(agent_ids, artifact_ids, protocol_name, judge, bus_latency)
    for op in ops:
        if op.op == "read":
    ...
    for protocol_name in protocols:
        for judge in judges:
            for regime in regimes:
                for scale in scales:
                    for seed in seeds:
                        for bus_latency in bus_latencies:
