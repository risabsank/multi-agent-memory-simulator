...
class DeprecatedJudge:
    def judge(
        self, *, previous, candidate_confidence: float, candidate_payload: dict
    ) -> ConflictDecision:
...
def test_deterministic_conflict_judge_parity_states() -> None:
    ...
    contested = judge.judge(
        previous=previous, candidate_confidence=0.5, candidate_payload={}
    )
    ...
    accepted = judge.judge(
        previous=previous, candidate_confidence=0.95, candidate_payload={}
    )
...
def test_eventual_protocol_uses_judge_for_sync_decision() -> None:
    ...
    sim = Simulator(
        ...
        protocol=EventualProtocol(
            propagation_delay=1, conflict_judge=DeprecatedJudge()
        ),
    )
    sim.schedule_write(t=0, agent_id="A", artifact_id=artifact_id, size=20)
    result = sim.run()
